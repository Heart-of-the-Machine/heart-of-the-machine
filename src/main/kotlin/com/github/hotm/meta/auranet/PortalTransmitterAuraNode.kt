...
class PortalTransmitterAuraNode(
...
) : AbstractMetaBlock(Type, access, updateListener, pos), DependantAuraNode, PortalTXAuraNode {

    companion object {
        private val NET_PARENT =
            MetaBlock.NET_ID.subType(
                PortalTransmitterAuraNode::class.java,
                HotMConstants.str("portal_transmitter_aura_node")
            )
    ...
    private fun findReceiver(): PortalRXAuraNode? {
        return if (!isClient) {
            ...
            val receiverPos = HotMPortalFinders.findNecterePortal(world, pos) {
                (it.getMetaBlock(server) as? PortalRXAuraNode)?.isValid() ?: false
            }

            cachedReceiver = receiverPos

            receiverPos?.getMetaBlock(server) as? PortalRXAuraNode
        } else {
            null
        }
    }
    ...
    object Type : MetaBlockType<PortalTransmitterAuraNode> {
