...
import com.github.hotm.meta.AbstractMetaBlock
...
class BasicSiphonAuraNode(
    ...
    childPos: BlockPos?
) : AbstractMetaBlock(Type, access, updateListener, pos), SiphonAuraNode, RenderedDependableAuraNode {

    companion object {
        private val NET_PARENT = MetaBlock.NET_ID.subType(
            BasicSiphonAuraNode::class.java,
            HotMConstants.str("basic_siphon_aura_node")
        )
    ...
    fun updateValue(value: Float, visitedNodes: MutableSet<DimBlockPos>) {
        ...
        MetaBlockUtils.getAt<DependantAuraNode>(childPos, access)?.recalculateDescendants(visitedNodes)
    }
    ...
    override fun wouldCauseDependencyLoop(
        ...
        return if (potentialAncestor == dimPos || visitedNodes.contains(dimPos)) {
            ...
            val res = MetaBlockUtils.getAt<DependantAuraNode>(childPos, access)
    ...
    override fun recalculateSiphonValue(chunkAura: Float, siphonCount: Int, visitedNodes: MutableSet<DimBlockPos>) {
        ...
        updateValue(SiphonAuraNodeUtils.calculateSiphonValue(10f, 2f, chunkAura, siphonCount), visitedNodes)
    ...
    object Type : MetaBlockType<BasicSiphonAuraNode> {
        ...
        override fun fromPacket(
            access: MetaAccess, pos: BlockPos, buf: NetByteBuf, ctx: IMsgReadCtx
        ): BasicSiphonAuraNode {
