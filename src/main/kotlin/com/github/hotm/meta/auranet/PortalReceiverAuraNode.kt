...
class PortalReceiverAuraNode(
...
) : AbstractMetaBlock(Type, access, updateListener, pos), RenderedDependableAuraNode,
    PortalRXAuraNode {

    companion object {
        private val NET_PARENT =
            MetaBlock.NET_ID.subType(PortalReceiverAuraNode::class.java, HotMConstants.str("portal_receiver_aura_node"))
    ...
    private fun findTransmitter(): PortalTXAuraNode? {
        return if (!isClient) {
            ...
            val transmitterPos = HotMPortalFinders.findNecterePortal(world, pos) {
                (it.getMetaBlock(server) as? PortalTXAuraNode)?.isValid() ?: false
            }

            cachedTransmitter = transmitterPos

            transmitterPos?.getMetaBlock(server) as? PortalTXAuraNode
        } else {
            null
        }
    }
    ...
    private fun getTransmitter(): PortalTXAuraNode? {
        if (!isClient) {
            ...
            cachedTransmitter?.let { cached ->
                val node = cached.getMetaBlock(server)
    ...
    object Type : MetaBlockType<PortalReceiverAuraNode> {
