...
class CollectorDistributorAuraNode(
...
) : AbstractMetaBlock(Type, access, updateListener, pos), DependantAuraNode, RenderedDependableAuraNode {

    companion object {
        private val NET_PARENT = MetaBlock.NET_ID.subType(
            CollectorDistributorAuraNode::class.java,
            HotMConstants.str("collector_distributor_aura_node")
        )
    ...
    private fun updateValue(value: Float, visitedNodes: MutableSet<DimBlockPos>) {
        ...
        for (child in children) {
            MetaBlockUtils.getAt<DependantAuraNode>(child, access)?.recalculateDescendants(visitedNodes)
        }
    }
    ...
    override fun wouldCauseDependencyLoop(
        ...
        return if (potentialAncestor == dimPos || visitedNodes.contains(dimPos)) {
            ...
            var res = false
            for (child in children) {
                val node = MetaBlockUtils.getAt<DependantAuraNode>(child, access)
    ...
    override fun recalculateDescendants(visitedNodes: MutableSet<DimBlockPos>) {
        ...
        updateValue(
            parents.stream().flatMap { StreamUtils.ofNullable(MetaBlockUtils.getAt<DependableAuraNode>(it, access)) }
    ...
    object Type : MetaBlockType<CollectorDistributorAuraNode> {
