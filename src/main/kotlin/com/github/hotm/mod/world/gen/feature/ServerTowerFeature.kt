...
class ServerTowerFeature(codec: Codec<ServerTowerConfig>) : Feature<ServerTowerConfig>(codec) {
    override fun place(ctx: FeatureContext<ServerTowerConfig>): Boolean {
        ...
        val border =
            (random.nextInt(config.maxBorder - config.minBorder + 1) + config.minBorder).coerceAtMost(size - size / 2 - 1)
                .coerceAtLeast(0)
